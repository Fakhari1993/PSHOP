...
{
    public partial class Frm_007_HavaleDialogMarjooiKharid : Form
    {
        public int state = 0;
        Decimal mjdi;
        ...
        public Frm_007_HavaleDialogMarjooiKharid(int id_faktor)
        {
            id_FaktorForoosh = id_faktor;
            InitializeComponent();
        }

        private void Frm_007_HavaleDialogMarjooiKharid_Load(object sender, EventArgs e)
        {
        ...
        private void buttonX1_Click(object sender, EventArgs e)
        {
            ...
            alldatabase.close();
            alldatabase.Connect();
            string id_pishfaktor = alldatabase.get_one_fiald("SELECT column01 FROM Table_021_MarjooiBuy WHERE columnid=" + id_FaktorForoosh);
            string id_girande = alldatabase.get_one_fiald("SELECT column03 FROM Table_021_MarjooiBuy WHERE columnid=" + id_FaktorForoosh);
            string tozihat = alldatabase.get_one_fiald("SELECT column04 FROM Table_021_MarjooiBuy WHERE columnid=" + id_FaktorForoosh);
            string zmansabt = alldatabase.get_one_fiald("SELECT getdate()");
            DataTable dt = alldatabase.get_list("SELECT * FROM Table_022_Child1_MarjooiBuy WHERE column01=" + id_FaktorForoosh);
            ...
            dr["column06"] = " فاکتور مرجوعی خرید شماره " + alldatabase.get_one_fiald("SELECT column01 FROM Table_021_MarjooiBuy WHERE columnid=" + id_FaktorForoosh);
            ...
            foreach (DataRow row in dt.Select())
            {
                ...
                alldatabase.close();
                alldatabase.Connect();
                string s = "UPDATE Table_022_Child1_MarjooiBuy SET column26={0},column25={1} WHERE columnid={2}";
            ...
            alldatabase.close();
            alldatabase.Connect();
            alldatabase.delete_update_all("UPDATE Table_007_PwhrsDraft SET  column19=" + id_FaktorForoosh.ToString()+"WHERE column01=" + code_havale.ToString());
