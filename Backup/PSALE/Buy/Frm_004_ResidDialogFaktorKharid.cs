...
{
    public partial class Frm_004_ResidDialogFaktorKharid : Form
    {
        public int state = 0;
        Decimal mjdi;
        ...
        string _idDrakhastKharid;
        db alldatabase = new db();
        public Frm_004_ResidDialogFaktorKharid(int id_faktorforooshmarjooi,string darkhast_id)
        {
            _idDrakhastKharid = darkhast_id;
            id_marjooiforoosh = id_faktorforooshmarjooi;
            InitializeComponent();
        }

        private void Frm_004_ResidDialogFaktorKharid_Load(object sender, EventArgs e)
        {
        ...
        private void buttonX1_Click(object sender, EventArgs e)
        {
            ...
            alldatabase.close();
            alldatabase.Connect();
            string id_pishfaktor = alldatabase.get_one_fiald("SELECT column01 FROM Table_015_BuyFactor WHERE columnid=" + id_marjooiforoosh);
            string id_girande = alldatabase.get_one_fiald("SELECT column03 FROM Table_015_BuyFactor WHERE columnid=" + id_marjooiforoosh);
            string tozihat = alldatabase.get_one_fiald("SELECT column06 FROM Table_015_BuyFactor WHERE columnid=" + id_marjooiforoosh);
            string zmansabt = alldatabase.get_one_fiald("SELECT getdate()");
            DataTable dt = alldatabase.get_list("SELECT * FROM Table_016_Child1_BuyFactor WHERE column01=" + id_marjooiforoosh);
            ...
            dr["column06"] = " فاکتور خرید شماره " + alldatabase.get_one_fiald("SELECT column01 FROM Table_015_BuyFactor WHERE columnid=" + id_marjooiforoosh);
            ...
            foreach (DataRow row in dt.Select())
            {
                ...
                string id_child_resid= alldatabase.get_one_fiald("SELECT columnid  FROM Table_012_Child_PwhrsReceipt WHERE column28=" + row["columnid"]);
                db.constr = Properties.Settings.Default.PSALE_ConnectionString;
                alldatabase.close();
                alldatabase.Connect();
                string s;
                if (_idDrakhastKharid != "")
                {
                    if (row["column24"].ToString() != "")
                    {
                        s = @"UPDATE Table_014_Child_RequestBuy SET column17={0},column18={1} WHERE columnid={2}";
                        s = string.Format(s, id_rsid, id_child_resid, row["column24"].ToString());
                 ...
                 s = "UPDATE Table_016_Child1_BuyFactor SET column26={0},column25={1} WHERE columnid={2}";
