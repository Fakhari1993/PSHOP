...
namespace PSALE._05_Sale
{
    public partial class Frm_005_SodoorSanadDialogFaktorKharid : Form
    {
        ...
        int _id_faktor;
        public Frm_005_SodoorSanadDialogFaktorKharid(int id_faktor)
        {
            _id_faktor = id_faktor;
            InitializeComponent();
        }

        private void Frm_005_SodoorSanadDialogFaktorKharid_Load(object sender, EventArgs e)
        {
            editBox3.Text = "فروش - صدور سند فاکتور خرید";
            faDatePicker1.SelectedDateTime = DateTime.Now;
            db.constr = Properties.Settings.Default.ACNT;
            alldatabase.close();
            alldatabase.Connect();
            ...
            db.constr = Properties.Settings.Default.SALE;
            alldatabase.close();
            alldatabase.Connect();

            dt_kasr = alldatabase.get_list(@"SELECT     dbo.Table_024_Discount.column01 AS name, SUM(dbo.Table_017_Child2_BuyFactor.column04) AS mablagh, dbo.Table_024_Discount.column05, 
...
FROM         dbo.Table_017_Child2_BuyFactor INNER JOIN
                      dbo.Table_024_Discount ON dbo.Table_017_Child2_BuyFactor.column02 = dbo.Table_024_Discount.columnid
WHERE     (dbo.Table_024_Discount.column02 =1) AND (dbo.Table_017_Child2_BuyFactor.column01 = " + _id_faktor + @")
GROUP BY dbo.Table_024_Discount.column01, dbo.Table_024_Discount.column05, dbo.Table_024_Discount.column06, dbo.Table_024_Discount.column07, 
            ...
            dt_ezafe = alldatabase.get_list(@"SELECT     dbo.Table_024_Discount.column01 AS name, SUM(dbo.Table_017_Child2_BuyFactor.column04) AS mablagh, dbo.Table_024_Discount.column05, 
                      dbo.Table_024_Discount.column06, dbo.Table_024_Discount.column07, dbo.Table_024_Discount.column08, dbo.Table_024_Discount.column09, 
                      dbo.Table_024_Discount.column10, dbo.Table_024_Discount.column11, dbo.Table_024_Discount.column12, dbo.Table_024_Discount.column13, 
                      dbo.Table_024_Discount.column14, dbo.Table_024_Discount.column15, dbo.Table_024_Discount.column16, dbo.Table_024_Discount.columnid
FROM         dbo.Table_017_Child2_BuyFactor INNER JOIN
                      dbo.Table_024_Discount ON dbo.Table_017_Child2_BuyFactor.column02 = dbo.Table_024_Discount.columnid
WHERE     (dbo.Table_024_Discount.column02 = 0) AND (dbo.Table_017_Child2_BuyFactor.column01 = " + _id_faktor + @")
GROUP BY dbo.Table_024_Discount.column01, dbo.Table_024_Discount.column05, dbo.Table_024_Discount.column06, dbo.Table_024_Discount.column07, 
                      dbo.Table_024_Discount.column08, dbo.Table_024_Discount.column09, dbo.Table_024_Discount.column10, dbo.Table_024_Discount.column11, 
                      dbo.Table_024_Discount.column12, dbo.Table_024_Discount.column13, dbo.Table_024_Discount.column14, dbo.Table_024_Discount.column15, 
                      dbo.Table_024_Discount.column16, dbo.Table_024_Discount.columnid");
            ...
            dt_faktor_h = alldatabase.get_list("SELECT * FROM Table_015_BuyFactor WHERE columnid=" + _id_faktor);

            dt_faktor_d = alldatabase.get_list(@"SELECT     dbo.Table_015_BuyFactor.column03 AS shakhs, dbo.Table_016_Child1_BuyFactor.column21 AS hazine, dbo.Table_016_Child1_BuyFactor.column22 AS proje, 
                      dbo.Table_015_BuyFactor.columnid AS idfaktor, SUM(dbo.Table_016_Child1_BuyFactor.column20) AS jam
...
GROUP BY dbo.Table_015_BuyFactor.column03, dbo.Table_016_Child1_BuyFactor.column21, dbo.Table_016_Child1_BuyFactor.column22, 
                      dbo.Table_015_BuyFactor.columnid
HAVING      (dbo.Table_015_BuyFactor.columnid =" + _id_faktor + ")");
        ...
        private void buttonX1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                try
                {
                    price = Int64.Parse(alldatabase.get_one_fiald("SELECT SUM(column20) FROM Table_016_Child1_BuyFactor WHERE column01=" + _id_faktor));
                ...
                alldatabase.close();
                alldatabase.Connect();
                id_person = alldatabase.get_one_fiald("SELECT column03 FROM Table_015_BuyFactor WHERE columnid=" + _id_faktor);
                string sharh = " فاکتور خرید شماره " + alldatabase.get_one_fiald("SELECT column01 FROM Table_015_BuyFactor WHERE columnid=" + _id_faktor);
                db.constr = Properties.Settings.Default.ACNT;
                alldatabase.close();
                alldatabase.Connect();
            ...
            // sabte bedehkar
                ...
                s = string.Format(s, sanadid.ToString(), multiColumnCombo1.DropDownList.GetValue("ACC_Code").ToString(),
                 ...
                 "0",price.ToString(), "19", _id_faktor, Class_BasicOperation._UserName, "getdate()", Class_BasicOperation._UserName, "getdate()");
                ...
                foreach (DataRow rows in dtdetals.Select())
                {
                    ...
                    rows["mablagh"], "0" ,  "19", _id_faktor, Class_BasicOperation._UserName, "getdate()", Class_BasicOperation._UserName, "getdate()");
                ...
                foreach (DataRow row in dt_ezafe.Select())
                {
                   ...
                   "0",row["mablagh"].ToString(),  "19", _id_faktor, Class_BasicOperation._UserName, "getdate()", Class_BasicOperation._UserName, "getdate()");
                ...
                foreach (DataRow row in dt_ezafe.Select())
                {
                   ...
                   row["mablagh"].ToString(), "0", "19", _id_faktor, Class_BasicOperation._UserName, "getdate()", Class_BasicOperation._UserName, "getdate()");
                ...
                foreach (DataRow row in dt_kasr.Select())
                {
                    ...
                    , "19", _id_faktor, Class_BasicOperation._UserName, "getdate()", Class_BasicOperation._UserName, "getdate()");
