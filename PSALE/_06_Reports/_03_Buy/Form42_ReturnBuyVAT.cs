...
{
    public partial class Form42_ReturnBuyVAT : Form
    {
        ...
        private void bt_Search_Click(object sender, EventArgs e)
        {
            string command = @"SELECT tpi.Column02 AS [name],
                                    ...
                                    (case when tcai.Column49=0  then 0 else  (( (tcsf.column11 - tcsf.column17) *(" + Convert.ToDecimal(Properties.Settings.Default.buytax) + @"))/100) end ) AS tax,
                                     (case when tcai.Column49=0  then 0 else (( (tcsf.column11 - tcsf.column17) *(" + Convert.ToDecimal(Properties.Settings.Default.buytaxes) + @"))/100)end ) AS taxes,
                                       tsf.column10 AS [havale],
                                       tsf.column11 AS [sanad]
                                FROM   Table_022_Child1_MarjooiBuy  tcsf
                                       JOIN  Table_021_MarjooiBuy tsf
                                ...
                                WHERE    tsf.column02>='" + faDatePickerStrip1.FADatePicker.Text + "' AND tsf.column02<='" + faDatePickerStrip2.FADatePicker.Text + "'  ";
            ...
            try
            {
                ...
                if (pageSetupDialog1.ShowDialog() == DialogResult.OK)
                    if (printDialog1.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        gridEXPrintDocument1.PageHeaderRight = "گزارش مالیات بر ارزش افزوده مرجوعی خرید";
