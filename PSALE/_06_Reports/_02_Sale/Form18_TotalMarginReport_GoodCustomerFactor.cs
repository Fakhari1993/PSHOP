...
{
    public partial class Form18_TotalMarginReport_GoodCustomerFactor : Form
    {
        ...
        public Form18_TotalMarginReport_GoodCustomerFactor()
        {
            InitializeComponent();
        }
        ...
        private void bt_Display_Click(object sender, EventArgs e)
        {
            if (faDatePickerStrip1.FADatePicker.SelectedDateTime.HasValue && faDatePickerStrip2.FADatePicker.SelectedDateTime.HasValue)
            {
                ...
                DataTable Table = clDoc.ReturnTable(ConSale.ConnectionString, @"SELECT f.GoodCode,
                                                                               SUM(f.TotalPrice) AS TotalPrice,
                                                                               SUM(f.TotalValue) AS TotalValue,
                                                                               SUM(f.Number) AS Number,
                                                                               (isnull(SUM(f.TotalPrice),0)-isnull(SUM(f.TotalValue),0))/nullif(SUM(f.Number),0)/nullif(isnull(SUM(f.TotalValue),0)/SUM(f.Number),0) as UMRatio,
                                                                              (
                                                                               (
                                                                        ...
                                                                        FROM   (
                                                                                   ...
                                                                                   FROM   (
                                                                                              SELECT ROW_NUMBER() OVER(
                                                                                                     ...
                                                                                                     dbo.Table_011_Child1_SaleFactor.column11 AS 
                                                                                                     TotalPrice,
