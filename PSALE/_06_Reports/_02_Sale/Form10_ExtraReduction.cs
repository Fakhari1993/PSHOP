...
{
    public partial class Form10_ExtraReduction : Form
    {
        ...
        private void Form10_ExtraReduction_Load(object sender, EventArgs e)
        {
            ...
            string[] Dates = Properties.Settings.Default.ExtraReduction.Split('-');
        ...
        private void bt_Display_Click(object sender, EventArgs e)
        {
            if (faDatePickerStrip1.FADatePicker.SelectedDateTime.HasValue && faDatePickerStrip2.FADatePicker.SelectedDateTime.HasValue)
            {
                ...
                SqlDataAdapter Adapter = new SqlDataAdapter(@"SELECT     dbo.Table_010_SaleFactor.columnid , dbo.Table_010_SaleFactor.column01 AS SaleNumber, dbo.Table_010_SaleFactor.column02 AS Date, 
                      dbo.Table_010_SaleFactor.column03 AS Customer, dbo.Table_010_SaleFactor.Column28 AS NetPrice, dbo.Table_010_SaleFactor.column17 AS Cancel, 
                      dbo.Table_010_SaleFactor.column19 AS Returned, dbo.Table_012_Child2_SaleFactor.column02 AS ExID, dbo.Table_012_Child2_SaleFactor.column03 AS [Percent], 
                      dbo.Table_012_Child2_SaleFactor.column04 AS Price,Table_010_SaleFactor.Column12
                        ...
                        WHERE     (dbo.Table_010_SaleFactor.column02 >= '{0}') AND (dbo.Table_010_SaleFactor.column02 <= '{1}')", ConSale);
                ...
                //Returned
                Adapter = new SqlDataAdapter(@"SELECT     dbo.Table_018_MarjooiSale.columnid , dbo.Table_018_MarjooiSale.column01 AS RNumber, dbo.Table_018_MarjooiSale.column02 AS RDate, 
                dbo.Table_018_MarjooiSale.column03 AS RCustomerId, dbo.Table_018_MarjooiSale.Column18 AS RNetPrice, dbo.Table_020_Child2_MarjooiSale.column02 AS RExID,
                dbo.Table_020_Child2_MarjooiSale.column03 AS RPercent, dbo.Table_020_Child2_MarjooiSale.column04 AS RPrice,Table_018_MarjooiSale.Column12
                FROM         dbo.Table_018_MarjooiSale INNER JOIN
                dbo.Table_020_Child2_MarjooiSale ON dbo.Table_018_MarjooiSale.columnid = dbo.Table_020_Child2_MarjooiSale.column01
                WHERE     (dbo.Table_018_MarjooiSale.column02 >= '{0}') AND (dbo.Table_018_MarjooiSale.column02 <='{1}')", ConSale);
        ...
        {
            if (faDatePickerStrip1.FADatePicker.SelectedDateTime.HasValue && faDatePickerStrip2.FADatePicker.SelectedDateTime.HasValue)
                Properties.Settings.Default.ExtraReduction = faDatePickerStrip1.FADatePicker.Text + "-" + faDatePickerStrip2.FADatePicker.Text;
        ...
        private void gridEX_ExtraRed_CurrentCellChanged(object sender, EventArgs e)
        {
            try
            {
                if (gridEX1.RowCount > 0)
                {
                    ...
                    bindingSource2.Filter = "RExID=" + gridEX1.GetValue("ColumnId").ToString();
                }
            }
            catch { }
        }
                    ...
                    if (UserScope.CheckScope(Class_BasicOperation._UserName, "Column11", 22))
                    {
                        foreach (Form item in Application.OpenForms)
                        {
                            if (item.Name == "Frm_013_ReturnFactor")
                            {
                                ...
                                frm.txt_Search.Text = gridEX_Returned.GetRow().Cells["Column01"].Text;
                                frm.bt_Search_Click(sender, e);
                                return;
                            }
                        }
                        _05_Sale.Frm_013_ReturnFactor frms = new _05_Sale.Frm_013_ReturnFactor(UserScope.CheckScope(Class_BasicOperation._UserName, "Column11", 23), Convert.ToInt32(gridEX_Returned.GetValue("columnid")));
                        try
                        {
                            frms.MdiParent = MainForm.ActiveForm;
                        }
        ...
        private void bt_PrintAll_Return_Click(object sender, EventArgs e)
        {
            if (this.gridEX1.RowCount > 0)
            {
                ...
                if (Table.Rows.Count > 0)
                {
                    _06_Reports.Form01_ReportForm frm = new Form01_ReportForm(Table, 30,   Date1, Date2, "فاکتورهای مرجوعی");
                    frm.ShowDialog();
                }
            }
        }
