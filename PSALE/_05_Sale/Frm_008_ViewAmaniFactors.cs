...
{
    public partial class Frm_008_ViewAmaniFactors : Form
    {
        ...
        public Frm_008_ViewAmaniFactors()
        {
            InitializeComponent();
        }
        ...
        private void Form_008_ViewSaleFactors_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                //*******************************************
                HeaderAdapter = new SqlDataAdapter(@"Select  * from Table_070_AmaniFactor  where Table_070_AmaniFactor.Column02>='" + faDatePickerStrip1.FADatePicker.Text + "' and Table_070_AmaniFactor.Column02<='" + faDatePickerStrip2.FADatePicker.Text + "'", ConSale);
                HeaderAdapter.Fill(dataSet1, "Header");

                Child1Adapter = new SqlDataAdapter(@"SELECT     dbo.Table_075_Child_AmaniFactor.*    FROM         dbo.Table_070_AmaniFactor INNER JOIN
                dbo.Table_075_Child_AmaniFactor ON dbo.Table_070_AmaniFactor.columnid = dbo.Table_075_Child_AmaniFactor.column01 WHERE     (dbo.Table_070_AmaniFactor.column02 >='" + faDatePickerStrip1.FADatePicker.Text + "' and dbo.Table_070_AmaniFactor.column02<='" + faDatePickerStrip2.FADatePicker.Text + "')", ConSale);
            ...
            try
            {
                if (this.gridEX_Header.RowCount > 0)
                {
                    if (UserScope.CheckScope(Class_BasicOperation._UserName, "Column11", 20))
                    {
                        ...
                        {
                            if (item.Name == "Frm_002_AmaniFaktor")
                            {
                                item.BringToFront();
                                Frm_002_AmaniFaktor frm = (Frm_002_AmaniFaktor)item;
                                frm.txt_Search.Text = gridEX_Header.GetRow().Cells["Column01"].Text;
                                frm.bt_Search_Click(sender, e);
                                return;
                            }
                        }
                        Frm_002_AmaniFaktor frms = new Frm_002_AmaniFaktor(UserScope.CheckScope(Class_BasicOperation._UserName, "Column11", 21), Convert.ToInt32(gridEX_Header.GetValue("columnid")));
                        try
                        {
