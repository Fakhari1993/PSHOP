...
namespace PSHOP._05_Sale
{
    ...
    {
        bool _del;
        int _ID = 0, ReturnId = 0, ReturnNum = 0, ResidId = 0, ResidNum = 0, _DraftID;
        ...
        DataTable DraftHeaderTbl = new DataTable();
        DataTable ChildDraftTbl = new DataTable();
        ...
        public Frm_002_Faktor(bool del, int ID)
        {
            _del = del;
            _ID = ID;
            InitializeComponent();
        }
        public Frm_002_Faktor(bool del, int DraftID, bool FromDraft)
        {
            _del = del;
            _DraftID = DraftID;
            InitializeComponent();
        ...
        private void Frm_002_PishFaktor_Load(object sender, EventArgs e)
        {
            ...
            (ConBase.ConnectionString, @"SELECT     dbo.Table_045_PersonInfo.ColumnId AS id, dbo.Table_045_PersonInfo.Column01 AS code, dbo.Table_045_PersonInfo.Column02 AS name, 
                      ...
                      dbo.Table_045_PersonInfo.Column30,Table_045_PersonInfo.Column07,dbo.Table_045_PersonInfo.Column146
...
WHERE     (dbo.Table_045_PersonInfo.Column12 = 1)");
            ...
            if (_FromDraft)
            {
                ...
                DraftHeaderTbl = clDoc.ReturnTable(ConWare.ConnectionString, "Select * from Table_007_PwhrsDraft where ColumnId=" + _DraftID);
                ChildDraftTbl = clDoc.ReturnTable(ConWare.ConnectionString, "Select * from Table_008_Child_PwhrsDraft where Column01=" + _DraftID);
                DataTable sefareshTbl = clDoc.ReturnTable(this.ConSale.ConnectionString, @"SELECT column08
                                                                                            FROM   Table_005_OrderHeader
                                                                                            WHERE  columnid = (
                                                                                                       SELECT tpd.column17
                                                                                                       FROM   " + ConWare.Database + ".dbo.Table_007_PwhrsDraft tpd WHERE tpd.columnid=" + _DraftID + @"
                                                                                                   )");


                //*********************Header********************************
                this.table_010_SaleFactorBindingSource.AddNew();
                DataRowView HeaderRow = (DataRowView)this.table_010_SaleFactorBindingSource.CurrencyManager.Current;
                HeaderRow["Column02"] = FarsiLibrary.Utils.PersianDate.Now.ToString("0000/00/00");
                HeaderRow["Column03"] = DraftHeaderTbl.Rows[0]["Column05"].ToString();
                HeaderRow["Column06"] = "فاکتور فروش صادر شده از حواله شماره " + DraftHeaderTbl.Rows[0]["Column01"] + " به تاریخ " + DraftHeaderTbl.Rows[0]["Column02"].ToString() + " به شماره عطف " + DraftHeaderTbl.Rows[0]["columnid"].ToString();
                HeaderRow["Column13"] = Class_BasicOperation._UserName;
                HeaderRow["Column14"] = Class_BasicOperation.ServerDate();
                HeaderRow["Column15"] = Class_BasicOperation._UserName;
                HeaderRow["Column16"] = Class_BasicOperation.ServerDate();
                HeaderRow["Column09"] = _DraftID;
                HeaderRow["Column12"] = DraftHeaderTbl.Rows[0]["Column23"].ToString();
                HeaderRow["Column40"] = (DraftHeaderTbl.Rows[0]["Column24"].ToString().Trim() == "" ? (object)DBNull.Value : DraftHeaderTbl.Rows[0]["Column24"].ToString());
                HeaderRow["Column41"] = DraftHeaderTbl.Rows[0]["Column25"].ToString();
                HeaderRow["Column42"] = DraftHeaderTbl.Rows[0]["column03"];
                HeaderRow["Column43"] = DraftHeaderTbl.Rows[0]["column04"];
                HeaderRow["Column61"] = 0;
                if (sefareshTbl.Rows.Count > 0 &&
                    sefareshTbl.Rows[0]["column08"] != DBNull.Value &&
                    sefareshTbl.Rows[0]["column08"] != null &&
                    !string.IsNullOrWhiteSpace(sefareshTbl.Rows[0]["column08"].ToString()))
                    HeaderRow["Column36"] = sefareshTbl.Rows[0]["column08"];
                if (HeaderRow["Column12"].ToString() == "False")
                {
                ...
                foreach (DataRow item in ChildDraftTbl.Rows)
                {
                    this.table_011_Child1_SaleFactorBindingSource.AddNew();
                    DataRowView Row = (DataRowView)this.table_011_Child1_SaleFactorBindingSource.CurrencyManager.Current;
                    ...
                    Row["Column11"] = item["Column11"].ToString();
                    Row["Column14"] = (item["Column23"].ToString().Trim() == "" ? (object)DBNull.Value : item["Column23"].ToString());
                    Row["Column15"] = item["Column24"].ToString().Trim();
                    Row["Column21"] = (item["Column13"].ToString().Trim() == "" ? (object)DBNull.Value : item["Column13"].ToString());
                    Row["Column22"] = (item["Column14"].ToString().Trim() == "" ? (object)DBNull.Value : item["Column14"].ToString());
                    Row["Column30"] = item["Column29"].ToString();
                    Row["Column34"] = (item["Column30"].ToString().Trim() == "" ? (object)DBNull.Value : item["Column30"].ToString());
                    Row["Column35"] = (item["Column31"].ToString().Trim() == "" ? (object)DBNull.Value : item["Column31"].ToString());

                    GoodbindingSource.DataSource = clGood.MahsoolInfo( 0);
                    GoodbindingSource.Filter = "GoodID=" + item["Column02"].ToString();
                    Row["Column31"] = ((DataRowView)GoodbindingSource.CurrencyManager.Current)["NumberInBox"].ToString();
                    Row["Column32"] = ((DataRowView)GoodbindingSource.CurrencyManager.Current)["NumberInPack"].ToString();

                    if (!Class_BasicOperation.CalLinearDis(int.Parse(DraftHeaderTbl.Rows[0]["Column05"].ToString())))
                    {
                        Row["Column16"] = ((DataRowView)GoodbindingSource.CurrencyManager.Current)["Discount"].ToString();
                        Row["Column18"] = ((DataRowView)GoodbindingSource.CurrencyManager.Current)["Extra"].ToString();
                    }
                    else
                    {
                        double[] array = clDoc.LastLinearDiscount(int.Parse(DraftHeaderTbl.Rows[0]["Column05"].ToString()), int.Parse(item["Column02"].ToString()));
                        Row["Column16"] = array[0];
                        Row["Column18"] = array[1];
                    }
                    ...
                    DataTable DisTbl = clDoc.ReturnTable(ConSale.ConnectionString, @"SELECT     Column01, Column04, Column05, Column06, SUM(Column07) AS SumDis
                                                        ...
                                                        WHERE     (Column02 <= N'" + HeaderRow["Column02"] + @"') AND (Column03 >= N'" + HeaderRow["Column02"] + @"')
                                                        and (Column01=" + HeaderRow["Column03"] + ") AND (Column05 <= " + item["Column07"].ToString() +
                       ") AND (Column06 >= " + item["Column07"].ToString() + ") AND (Column04=" + item["Column02"].ToString() + ") GROUP BY Column01, Column04, Column05, Column06");
                    if (DisTbl.Rows.Count > 0)
                    {
                        Row["Column16"] = DisTbl.Rows[0]["SumDis"].ToString();
                    }



                    Row["Column36"] = item["Column34"].ToString();
                    Row["Column37"] = item["Column35"].ToString();
                    Row["Column38"] = (item["Column36"].ToString().Trim() == "" ? (object)DBNull.Value : item["Column36"].ToString());
                    Row["Column39"] = (item["Column37"].ToString().Trim() == "" ? (object)DBNull.Value : item["Column37"].ToString());


                    //اگر فاکتور ارزی بود تمام قیمتها بر اساس ریال میشود
                    if (DraftHeaderTbl.Rows[0]["Column23"].ToString() == "True")
                    {
                        Double jam, takhfif, ezafe;
                        jam = Convert.ToDouble(Row["Column11"].ToString());
                        takhfif = (jam * (Convert.ToDouble(Row["Column16"].ToString()) / 100));
                        ezafe = ((jam - takhfif) *
                            (Convert.ToDouble(Row["Column18"].ToString()) / 100));
                        Row["Column17"] = takhfif;
                        Row["column19"] = ezafe;
                        Row["column20"] = (jam - takhfif) + ezafe;
                        Row["column40"] = (jam - takhfif);
                    }
                    else
                    {
                        //در غیر این صورت بر اساس اعداد صحیح
                        Int64 jam, takhfif, ezafe;
                        jam = Convert.ToInt64(Convert.ToDouble(Row["Column11"].ToString()));
                        takhfif = Convert.ToInt64(Convert.ToDouble(Row["Column11"].ToString())
                            * Convert.ToDouble(Row["Column16"].ToString()) / 100);
                        ezafe = Convert.ToInt64(
                            //جمع
                            (Convert.ToDouble(Row["Column11"].ToString()) -
                            //تخفیف
                            (Convert.ToDouble(Row["Column11"].ToString()) * Convert.ToDouble(Row["Column16"].ToString()) / 100)) *
                            //درصد
                          Convert.ToDouble(Row["Column18"].ToString())
                          / 100);
                    ...
                    if (Convert.ToBoolean(item["Column29"]))
                    {
                        if (global::PWHRS.Properties.Settings.Default.Gift100Darsad)
                        {
                ...
                if (DraftHeaderTbl.Rows[0]["Column21"].ToString().Trim() != "" && DraftHeaderTbl.Rows[0]["Column21"].ToString().Trim() != "0")
                {
                    string Kind = clDoc.ExScalar(ConSale.ConnectionString, "Table_024_Discount", "Column02",
                            "ColumnId", DraftHeaderTbl.Rows[0]["Column21"].ToString());
                    this.table_012_Child2_SaleFactorBindingSource.AddNew();
                    DataRowView Row = (DataRowView)this.table_012_Child2_SaleFactorBindingSource.CurrencyManager.Current;
                    Row["Column02"] = DraftHeaderTbl.Rows[0]["Column21"].ToString();
                    Row["Column03"] = 0;
                    Row["Column04"] = DraftHeaderTbl.Rows[0]["Column22"].ToString();
                    Row["Column05"] = (Kind == "True" ? true : false);
                    Row["Column06"] = DBNull.Value;
                ...
                txt_TotalPrice.Value = Convert.ToDouble(gridEX_List.GetTotal(gridEX_List.RootTable.Columns["Column20"], Janus.Windows.GridEX.AggregateFunction.Sum).ToString());
                ...
                try
                {
                    SalePrice = UserScope.CheckScope(Class_BasicOperation._UserName, "Column10", 142);
                }
                catch
                {
                }
            ...
            try
            {
                ...
                foreach (Janus.Windows.GridEX.GridEXColumn item in gridEX1.RootTable.Columns)
                {
                    if (item.Key == "column07" || item.Key == "column08" || item.Key == "Column09" || item.Key == "Column10" ||
                        item.Key == "column17" || item.Key == "column19" || item.Key == "column20" || item.Key == "Column61"
                        || item.Key == "Column62" || item.Key == "Column63"
                         )
        ...
        private void Save_Event(object sender, EventArgs e)
        {
               ...
               gridEX1.GetRow().Cells["Column03"].Text.Trim() != "")
            {
                ...
                if (Convert.ToBoolean(Row["Column61"]))
                {
                ...
                if (_FromDraft)
                {
                    clDoc.RunSqlCommand(ConWare.ConnectionString, "UPDATE Table_007_PwhrsDraft SET Column16=" + Row["ColumnId"].ToString() + " where ColumnId=" + _DraftID);
        ...
        public void bt_Del_Click(object sender, EventArgs e)
        {
            if (this.table_010_SaleFactorBindingSource.Count > 0)
            {
                try
                {
                    ...
                    string RowID = ((DataRowView)this.table_010_SaleFactorBindingSource.CurrencyManager.Current)["ColumnId"].ToString();
                    if (Convert.ToBoolean(((DataRowView)this.table_010_SaleFactorBindingSource.CurrencyManager.Current)["Column61"]))
                    {
        ...
        private void table_010_SaleFactorBindingSource_PositionChanged(object sender, EventArgs e)
        {
            try
            {
                //try
                //{
                //    DataTable GoodTable = clGood.MahsoolInfoForNewFactor(gridEX1.GetValue("Column02").ToString(), gridEX1.GetValue("Column42"));
                //    GoodbindingSource.DataSource = clGood.MahsoolInfoForFactor(null, null);
                        ...
                        //if (!UserScope.CheckScope(Class_BasicOperation._UserName, "Column11", 171))
                        {
                            foreach (Janus.Windows.GridEX.GridEXColumn item in gridEX1.RootTable.Columns)
                            {
                                if (item.Key == "column06" || item.Key == "column13" ||
                                    item.Key == "Column40" || item.Key == "Column41" || item.Key == "column12"
                                    || (item.Key == "column05")
                                    || item.Key == "column21" || item.Key == "column22" || item.Key == "column23" || item.Key == "column24"
        ...
        private void InvertDoc(DataRowView Row)
        {
            ...
            if (PreDoc.Rows.Count > 0)
            {
              ...
              ,[Column18]      ,[Column19]      ,[Column20]      ,[Column21]      ,[Column22],Column27) 
                ...
                //DataTable Table = clDoc.ReturnTable(ConAcnt.ConnectionString, "Select * from Table_065_SanadDetail where Column00=@Key and Column16=27");
        ...
        private void mnu_GoodInformation_Click(object sender, EventArgs e)
        {
            if (UserScope.CheckScope(Class_BasicOperation._UserName, "Column11", 5))
            {
                ...
                DataTable GoodTable = (Properties.Settings.Default.ShowMojodi ? clGood.MahsoolInfoForNewFactor(null, null) : clGood.MahsoolInfo( 0));
                gridEX_List.DropDowns["GoodCode"].SetDataBinding(GoodTable, "");
                gridEX_List.DropDowns["GoodName"].SetDataBinding(GoodTable, "");
            }
            else
...
from Table_010_SaleFactor where Column07=" + _PrefactorID + "),0)", Con);
            ...
            try
            {
                ...
                if (e.Column.Key == "column03")
                {
                    ...
                    gridEX1.SetValue("Column36", (((DataRowView)gridEX1.RootTable.Columns["Column03"].DropDown.FindItem(gridEX1.GetValue("Column03")))["Column30"]!=null &&
                        ((DataRowView)gridEX1.RootTable.Columns["Column03"].DropDown.FindItem(gridEX1.GetValue("Column03")))["Column30"].ToString().Trim() == "" ? DBNull.Value :
                    ...
                    try
                    {
                        ...
                        {
                            item.BeginEdit();
                            item.Cells["column07"].Value = gridEX1.GetValue("column03");
                            item.EndEdit();
                        }
        ...
        private void gridEX_List_CellUpdated(object sender, ColumnActionEventArgs e)
        {
            ...
            try
            {
                ...
                if (gridEX_List.GetValue("column04") == DBNull.Value || gridEX_List.GetValue("column04") == null || string.IsNullOrWhiteSpace(gridEX_List.GetValue("column04").ToString()))
                    gridEX_List.SetValue("column04", 0);
                if (gridEX_List.GetValue("column05") == DBNull.Value || gridEX_List.GetValue("column05") == null || string.IsNullOrWhiteSpace(gridEX_List.GetValue("column05").ToString()))
                    gridEX_List.SetValue("column05", 0);
                if (gridEX_List.GetValue("column06") == DBNull.Value || gridEX_List.GetValue("column06") == null || string.IsNullOrWhiteSpace(gridEX_List.GetValue("column06").ToString()))
                    gridEX_List.SetValue("column06", 0);
                if (gridEX_List.GetValue("tedaddarkartoon") == DBNull.Value || gridEX_List.GetValue("tedaddarkartoon") == null || string.IsNullOrWhiteSpace(gridEX_List.GetValue("tedaddarkartoon").ToString()))
                    gridEX_List.SetValue("tedaddarkartoon", 0);
                if (gridEX_List.GetValue("tedaddarbaste") == DBNull.Value || gridEX_List.GetValue("tedaddarbaste") == null || string.IsNullOrWhiteSpace(gridEX_List.GetValue("tedaddarbaste").ToString()))
                    gridEX_List.SetValue("tedaddarbaste", 0);
                //محاسبه تعداد کل
        ...
        private void bt_SettleFactor_Click(object sender, EventArgs e)
        {
            if (this.table_010_SaleFactorBindingSource.Count > 0)
            {
                if (UserScope.CheckScope(Class_BasicOperation._UserName, "Column11", 165))
                {
                    ...
                    {
                        string RowID = ((DataRowView)this.table_010_SaleFactorBindingSource.CurrencyManager.Current)["ColumnId"].ToString();
                        //به درخواست مشتری نگار سرام و دستور مهندس کامنت شد
                        //if (clDoc.OperationalColumnValue("Table_010_SaleFactor", "Column10", RowID) == 0)
        ...
        private void gridEX_Extra_CurrentCellChanging(object sender, CurrentCellChangingEventArgs e)
        {
        ...
        private void btn_ConfirmFactor_Click(object sender, EventArgs e)
        {
            if (this.table_010_SaleFactorBindingSource.Count > 0)
            {

                if (Convert.ToBoolean(((DataRowView)this.table_010_SaleFactorBindingSource.CurrencyManager.Current)["column17"])
                    || Convert.ToBoolean(((DataRowView)this.table_010_SaleFactorBindingSource.CurrencyManager.Current)["column19"])
                    )
                {
                ...
                string command = string.Empty;
                Class_UserScope UserScope = new Class_UserScope();
                if (!UserScope.CheckScope(Class_BasicOperation._UserName, "Column11", 236))
                {
                ...
                if (Convert.ToBoolean(((DataRowView)this.table_010_SaleFactorBindingSource.CurrencyManager.Current)["Column61"]))
                {
                    if (DialogResult.Yes == MessageBox.Show("آیا مایل به لغو تائید فاکتور هستید؟", "", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1))
                        ...
                        command = "UPDATE " + ConSale.Database + ".dbo.Table_010_SaleFactor set Column61=0,Column62='" + Class_BasicOperation._UserName + "',Column63=getdate()  where columnid=" + RowID;
                ...
                else
                {
                    ...
                    if (DialogResult.Yes == MessageBox.Show("آیا مایل به تائید فاکتور هستید؟", "", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1))
                        ...
                        command = "UPDATE " + ConSale.Database + ".dbo.Table_010_SaleFactor set Column61=1,Column62='" + Class_BasicOperation._UserName + "',Column63=getdate() where columnid=" + RowID;
                }
