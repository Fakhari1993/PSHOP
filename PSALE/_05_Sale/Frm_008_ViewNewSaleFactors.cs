...
{
    public partial class Frm_008_ViewNewSaleFactors : Form
    {
        SqlConnection ConSale = new SqlConnection(Properties.Settings.Default.SALE);
        SqlConnection ConWare = new SqlConnection(Properties.Settings.Default.WHRS);
        SqlConnection ConBase = new SqlConnection(Properties.Settings.Default.BASE);
        SqlConnection ConAcnt = new SqlConnection(Properties.Settings.Default.ACNT);
        SqlDataAdapter HeaderAdapter, Child1Adapter, Child2Adapter, Child3Adapter;
        Class_UserScope UserScope = new Class_UserScope();
        Classes.Class_Documents clDoc = new Classes.Class_Documents();
        bool _BackSpace = false;

        public Frm_008_ViewNewSaleFactors()
        {
            InitializeComponent();
        }
        ...
        private void Form_008_ViewSaleFactors_Load(object sender, EventArgs e)
        {
            ...
            SqlDataAdapter Adapter = new SqlDataAdapter("Select columnid,column01 from Table_005_OrderHeader", ConSale);
            Adapter.Fill(dataSet1, "Order");
            gridEX_Header.DropDowns["Order"].SetDataBinding(dataSet1.Tables["Order"], "");
            ...
            try
            {
                gridEX_List.DropDowns["MainGroup"].DataSource = clDoc.ReturnTable(ConWare.ConnectionString, @"SELECT tcai.columnid,tmg.column02 From  table_004_CommodityAndIngredients tcai join  table_002_MainGroup tmg   ON  tmg.columnid = tcai.column03");
            }
            catch
            {
            }
            gridEX_List.DropDowns["SubGroup"].DataSource = clDoc.ReturnTable(this.ConWare.ConnectionString, @"SELECT tcai.columnid,tsg.column03 From table_004_CommodityAndIngredients tcai   JOIN dbo.table_003_SubsidiaryGroup tsg
...
isnull(" + ConWare.Database + @".dbo.Table_007_PwhrsDraft.Column22,0) as CarryPrice,' ' as EditDate ,  tpi.Column02 AS Dealler,
       tsfd.Column03 AS Profit,
       tsfd.Column04 AS InsertUser,
       tsfd.Column05 AS InsertTime,
       tsfd.Column06 AS EditUser,
       tsfd.Column07 EditTime
from Table_010_SaleFactor
 LEFT JOIN Table_012_SaleFactorDealler tsfd
            ON  tsfd.Column01 = Table_010_SaleFactor.columnid
       LEFT JOIN " +ConBase.Database+@".dbo.Table_045_PersonInfo tpi
            ON  tpi.ColumnId = tsfd.Column02
left outer join " + ConWare.Database + @".dbo.Table_007_PwhrsDraft
on Table_010_SaleFactor.column09=" + ConWare.Database + ".dbo.Table_007_PwhrsDraft.columnid where Table_010_SaleFactor.Column02>='" + faDatePickerStrip1.FADatePicker.Text + "' and Table_010_SaleFactor.Column02<='" + faDatePickerStrip2.FADatePicker.Text + "'  ", ConSale);
            ...
            dbo.Table_011_Child1_SaleFactor ON dbo.Table_010_SaleFactor.columnid = dbo.Table_011_Child1_SaleFactor.column01 WHERE     (dbo.Table_010_SaleFactor.column02 >='" + faDatePickerStrip1.FADatePicker.Text + "' and dbo.Table_010_SaleFactor.column02<='" + faDatePickerStrip2.FADatePicker.Text + "')  ", ConSale);
            Child1Adapter.Fill(dataSet1, "Child1");

            Child2Adapter = new SqlDataAdapter(@" SELECT dbo.Table_012_Child2_SaleFactor.* 
                                        FROM   dbo.Table_010_SaleFactor
                                               INNER JOIN dbo.Table_012_Child2_SaleFactor
                                                    ON  dbo.Table_010_SaleFactor.columnid = dbo.Table_012_Child2_SaleFactor.column01
                                                
                                        WHERE  (
                                                   dbo.Table_010_SaleFactor.column02 >=
                                                   '" + faDatePickerStrip1.FADatePicker.Text + @"'
                                                   AND dbo.Table_010_SaleFactor.column02 <=
            ...
            Child3Adapter = new SqlDataAdapter(@" SELECT tsfs.ColumnId,
                                                           tsfs.Column01,
                                                           tpi.Column02 ,
                                                           tsfs.Column03 ,
                                                           tsfs.Column04,
                                                           ...
                                                           tsfs.Column07
                                                    FROM dbo.Table_010_SaleFactor
                                               INNER JOIN  Table_012_SaleFactorSeller tsfs ON dbo.Table_010_SaleFactor.columnid =tsfs.Column01
                                                           JOIN " + ConBase.Database+@".dbo.Table_045_PersonInfo tpi
                                                                ON  tpi.ColumnId = tsfs.Column02  WHERE  (
                                                   dbo.Table_010_SaleFactor.column02 >=
                                                   '" + faDatePickerStrip1.FADatePicker.Text + @"'
                                                   AND dbo.Table_010_SaleFactor.column02 <=
                                                       '" + faDatePickerStrip2.FADatePicker.Text + @"'
                                               ) ", ConSale);
            Child3Adapter.Fill(dataSet1, "Child3");
            ...
            DataRelation Relation3 = new DataRelation("R_Header_Child3", dataSet1.Tables["Header"].Columns["ColumnId"], dataSet1.Tables["Child3"].Columns["Column01"]);
            ...
            ForeignKeyConstraint Fkc3 = new ForeignKeyConstraint("F_Header_Child3", dataSet1.Tables["Header"].Columns["ColumnId"], dataSet1.Tables["Child3"].Columns["Column01"]);
            ...
            gridEX_Extra.DataMember = "R_Header_Child3";
          
 
            gridEX_Header.Select();
            gridEX_Header.Focus();
            gridEX_Header.Row = gridEX_Header.FilterRow.Position;
            foreach (Janus.Windows.GridEX.GridEXRow item1 in this.gridEX_Header.GetDataRows())
            {
                item1.BeginEdit();
                if (item1.Cells["column16"].Value != null && !string.IsNullOrWhiteSpace(item1.Cells["column16"].Value.ToString()))
                    item1.Cells["EditDate"].Value = FarsiLibrary.Utils.PersianDateConverter.ToPersianDate(Convert.ToDateTime(item1.Cells["column16"].Value)).ToString();
        ...
        private void bt_Search_Click(object sender, EventArgs e)
        {
            if (faDatePickerStrip1.FADatePicker.SelectedDateTime.HasValue && faDatePickerStrip2.FADatePicker.SelectedDateTime.HasValue && faDatePickerStrip2.FADatePicker.Text.Length == 10 && faDatePickerStrip1.FADatePicker.Text.Length == 10)
            {
                ...
                dataSet1.Tables["Child3"].Clear();

                HeaderAdapter = new SqlDataAdapter(@"Select Table_010_SaleFactor.*,
                isnull(" + ConWare.Database + @".dbo.Table_007_PwhrsDraft.Column22,0) as CarryPrice,  tpi.Column02 AS Dealler,
       tsfd.Column03 AS Profit,
       tsfd.Column04 AS InsertUser,
       tsfd.Column05 AS InsertTime,
       tsfd.Column06 AS EditUser,
       tsfd.Column07 EditTime from Table_010_SaleFactor
 LEFT JOIN Table_012_SaleFactorDealler tsfd
            ON  tsfd.Column01 = Table_010_SaleFactor.columnid
       LEFT JOIN " + ConBase.Database + @".dbo.Table_045_PersonInfo tpi
            ON  tpi.ColumnId = tsfd.Column02
            ...
            try
            {
                if (this.gridEX_Header.RowCount > 0)
                {
                    if (UserScope.CheckScope(Class_BasicOperation._UserName, "Column11", 20))
                    {
                        ...
                        {
                            if (item.Name == "Frm_002_NewFaktor")
                            {
                                item.BringToFront();
                                Frm_002_NewFaktor frm = (Frm_002_NewFaktor)item;
                                frm.txt_Search.Text = gridEX_Header.GetRow().Cells["Column01"].Text;
                                frm.bt_Search_Click(sender, e);
                                return;
                            }
                        }
                        Frm_002_NewFaktor frms = new Frm_002_NewFaktor(UserScope.CheckScope(Class_BasicOperation._UserName, "Column11", 21), Convert.ToInt32(gridEX_Header.GetValue("columnid")));
                        try
                        {
                ...
                try
                {
                    if (this.gridEX_Header.GetCheckedRows().Length > 0)
                    {
                        if (UserScope.CheckScope(Class_BasicOperation._UserName, "Column11", 128))
                        {
                            ...
                            dbo.Table_045_PersonInfo ON dbo.Table_065_CityInfo.Column01 = dbo.Table_045_PersonInfo.Column21 where ColumnId=" + Row.Cells["Column03"].Value.ToString());
                                if (CustomerTable.Rows.Count == 0)
                                {
                                    Class_BasicOperation.ShowMsg("", "اطلاعات مشتری در فاکتور صحیح نیست", "Stop");
                                    return;
                                }
                            }
                            ...
                            dbo.Table_045_PersonInfo ON dbo.Table_065_CityInfo.Column01 = dbo.Table_045_PersonInfo.Column21 where ColumnId=" + Row.Cells["Column03"].Value.ToString());
                                        ...
                                        HeaderTable.Rows.Add(
                                                        ...
                                                        Row.Cells["Column37"].Text,
                                                        Row.Cells["Column02"].Value,
                                                        null,
                                                        Row.Cells["Column03"].Value,
                                                        Row.Cells["Column03"].Text,
                                                      ...
                                                      (Row.Cells["Column12"].Value.ToString() == "True" ? "***فاکتور ارزی***" : "***فاکتور ریالی***"),
                                                       ...
                                                       Row.Cells["Column21"].Value.ToString(),
                                                       NetTotal,
                                                       Child1.Cells["Column18"].Value.ToString(),
                                                       Row.Cells["Column09"].Text,
                                                       Child1.Cells["Column37"].Value.ToString(),
                                                       Child1.Cells["Column23"].Text.ToString(),
                                                       null,
                                                       Row.Cells["Column10"].Value.ToString(),
                                                       null,
                                                       null,
                                                       null,
                                                       null,
                                                       null,
                                                       FarsiLibrary.Utils.PersianDate.Now.ToString("####/##/##") 
                                                       );
                                ...
                                _05_Sale.Reports.Form_TotalSaleFactorPrint frm = new Reports.Form_TotalSaleFactorPrint(HeaderTable, DetailTable, false);
                                frm.ShowDialog();
