...
{
    public partial class Form03_OrderAppendix : Form
    {
        int _HeaderId,_HeaderNum;

        public Form03_OrderAppendix(int HeaderId, int HeaderNum)
        {
            InitializeComponent();
            _HeaderId = HeaderId;
        ...
        {
            this.table_68_OrderAttachmentsTableAdapter.Fill(this.dataSet_Sale4.Table_68_OrderAttachments, _HeaderId);
        ...
        {
            if (this.table_68_OrderAttachmentsBindingSource.Count > 0)
            {
                try
                {
                    this.Validate();
                    this.table_68_OrderAttachmentsBindingSource.EndEdit();
                    this.table_68_OrderAttachmentsTableAdapter.Update(this.dataSet_Sale4.Table_68_OrderAttachments);
...

                {
                    if (DialogResult.Yes == MessageBox.Show("آیا مایل به حذف پیوست(ها) هستید؟", "", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1, MessageBoxOptions.RightAlign))
                    {
                        try
                        {
                            ...
                            {
                                this.table_68_OrderAttachmentsBindingSource.RemoveCurrent();
                                this.table_68_OrderAttachmentsBindingSource.EndEdit();
                                this.table_68_OrderAttachmentsTableAdapter.Update(this.dataSet_Sale4.Table_68_OrderAttachments);
