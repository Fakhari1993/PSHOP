...
{
    public partial class Frm_038_ViewReturnAmaniFactors : Form
    {
        ...
        public Frm_038_ViewReturnAmaniFactors()
        {
            InitializeComponent();
        }
        ...
        private void Form_008_ViewSaleFactors_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                //*******************************************
                HeaderAdapter = new SqlDataAdapter(@"Select  * from Table_080_ReturnAmani  where Table_080_ReturnAmani.Column02>='" + faDatePickerStrip1.FADatePicker.Text + "' and Table_080_ReturnAmani.Column02<='" + faDatePickerStrip2.FADatePicker.Text + "'", ConSale);
                HeaderAdapter.Fill(dataSet1, "Header");

                Child1Adapter = new SqlDataAdapter(@"SELECT     dbo.Table_085_ReturnAmaniChild.*    FROM         dbo.Table_080_ReturnAmani INNER JOIN
                dbo.Table_085_ReturnAmaniChild ON dbo.Table_080_ReturnAmani.columnid = dbo.Table_085_ReturnAmaniChild.column01 WHERE     (dbo.Table_080_ReturnAmani.column02 >='" + faDatePickerStrip1.FADatePicker.Text + "' and dbo.Table_080_ReturnAmani.column02<='" + faDatePickerStrip2.FADatePicker.Text + "')", ConSale);
            ...
            try
            {
                if (this.gridEX_Header.RowCount > 0)
                {
                    if (UserScope.CheckScope(Class_BasicOperation._UserName, "Column11", 20))
                    {
                        ...
                        {
                            if (item.Name == "Frm_037_ReturnAmaniFaktor")
                            {
                                item.BringToFront();
                                Frm_037_ReturnAmaniFaktor frm = (Frm_037_ReturnAmaniFaktor)item;
                                frm.txt_Search.Text = gridEX_Header.GetRow().Cells["Column01"].Text;
                                frm.bt_Search_Click(sender, e);
                                return;
                            }
                        }
                        Frm_037_ReturnAmaniFaktor frms = new Frm_037_ReturnAmaniFaktor(UserScope.CheckScope(Class_BasicOperation._UserName, "Column11", 21), Convert.ToInt32(gridEX_Header.GetValue("columnid")));
                        try
                        {
