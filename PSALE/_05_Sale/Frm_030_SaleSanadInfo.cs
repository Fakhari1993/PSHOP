    ...
    public partial class Frm_030_SaleSanadInfo : Form
    {
        ...
        private void Btn_Save_Click(object sender, EventArgs e)
        {
           ...
           ,[Column35]) VALUES(" + HavaleID + "," + item1["Column02"].ToString() + "," +
                            ...
                            foreach (DataRow item2 in Table.Rows)
                            {
                                ...
                                {
                                    Adapter = new SqlDataAdapter("EXEC	 " + (Class_BasicOperation._WareType ? "[dbo].[PR_00_NewFIFO] " : " [dbo].[PR_05_AVG] ") + "  @GoodParameter = " + item2["Column02"].ToString() + ", @WareCode = " + SaleRow["Column42"].ToString(), Con);
                ...
                VALUES((Select Isnull((Select Max(Column00) from Table_060_SanadHead),0))+1,'" + faDatePicker1.Text + "',2,0,'" + txt_Cover.Text + "','" + Class_BasicOperation._UserName +
                        ...
                        if (Convert.ToInt32(DocID.Value) > 0)
                        {
                            ...
                            foreach (DataRow dr1 in dt.Rows)
                            {
                                ...
                                ///تخفیف
                                ///
                                if (dr1["Kosoorat"] != null &&
                                    dr1["Kosoorat"].ToString() != string.Empty &&
                                    Convert.ToDouble(dr1["Kosoorat"]) > Convert.ToDouble(0))
                                {

                                    _AccInfo = clDoc.ACC_Info(dr1["Bed"].ToString());
                                    clDoc.ExportDoc_Detail(Convert.ToInt32(DocID.Value), dr1["Bed"].ToString(), Int16.Parse(_AccInfo[0].ToString()),
                                                       _AccInfo[1].ToString(), _AccInfo[2].ToString(), _AccInfo[3].ToString(), _AccInfo[4].ToString(),
                                                       null,
                                                         null,
                                                        null,
                                                       " تخفیف فاکتور فروش ش" + dt.Rows[0]["Column01"].ToString(),
                                                      Convert.ToInt64(Math.Round(Convert.ToDouble(dr1["Kosoorat"].ToString()), 3))
                                                       , 0
                                                       , 0,
                                                      0,
                                                       -1,
                                    ...
                                    _AccInfo = clDoc.ACC_Info(dr1["Bes"].ToString());
                                    clDoc.ExportDoc_Detail(Convert.ToInt32(DocID.Value), dr1["Bes"].ToString(), Int16.Parse(_AccInfo[0].ToString()),
                                                       _AccInfo[1].ToString(), _AccInfo[2].ToString(), _AccInfo[3].ToString(), _AccInfo[4].ToString()
                                                       , SaleRow["column03"].ToString(),
                                                         null,
                                                        null,
                                ...
                                //ارزش افزوده
                                if (dr1["Ezafat"] != null &&
                                    dr1["Ezafat"].ToString() != string.Empty &&
                                    Convert.ToDouble(dr1["Ezafat"]) > 0)
                                {
                                    ...
                                    clDoc.ExportDoc_Detail(Convert.ToInt32(DocID.Value), dr1["Bes"].ToString(), Int16.Parse(_AccInfo[0].ToString()),
                                                       ...
                                                       "ارزش افزوده فاکتور فروش ش" + dt.Rows[0]["Column01"].ToString(),
                                                         0
                                                     , Convert.ToInt64(Math.Round(Convert.ToDouble(dr1["Ezafat"].ToString()), 3))
                                                       , 0,
                                                      0,
                                                      -1,
                                                          15, int.Parse(dt.Rows[0]["ColumnId"].ToString()), Class_BasicOperation._UserName,
        ...
        private void CheckEssentialItems(object sender, EventArgs e)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                {
                    All_Controls_Row1(dr["Bed"].ToString(), int.Parse(SaleRow["column03"].ToString()), null, null);
                    ...
                    TPerson.Rows.Add(Int32.Parse(SaleRow["column03"].ToString()), dr["Bed"].ToString(), Convert.ToDouble(dr["Ezafat"]));


                }
                if (Convert.ToDouble(dr["Kosoorat"]) > 0)
                {
                    All_Controls_Row1(dr["Bes"].ToString(), int.Parse(SaleRow["column03"].ToString()), null, null);
                    ...
                    TPerson.Rows.Add(Int32.Parse(SaleRow["column03"].ToString()), dr["Bes"].ToString(), Convert.ToDouble(dr["Kosoorat"]));
        ...
        private float FirstRemain(int GoodCode)
        {
            using (SqlConnection Con = new SqlConnection(Properties.Settings.Default.WHRS))
            {
                ...
                string CommandText = @"SELECT     ISNULL(SUM(InValue) - SUM(OutValue),0) AS Remain
                       ...
                       WHERE     (Date <= '{2}')";
                CommandText = string.Format(CommandText, SaleRow["Column42"], GoodCode, SaleRow["column02"]);
