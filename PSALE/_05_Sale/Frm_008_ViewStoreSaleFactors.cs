...
{
    public partial class Frm_008_ViewStoreSaleFactors  : Form
    {
        ...
        public Frm_008_ViewStoreSaleFactors ()
        {
            InitializeComponent();
        }
        ...
        private void Form_008_ViewSaleFactors_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                //gridEX_Header.DropDowns["ware"].SetDataBinding(clDoc.ReturnTable(ConWare.ConnectionString, "Select * from Table_001_PWHRS"), "");
                ...
                //            dbo.Table_012_Child2_SaleFactor ON dbo.Table_010_SaleFactor.columnid = dbo.Table_012_Child2_SaleFactor.column01 WHERE     (dbo.Table_010_SaleFactor.column02 >='" + faDatePickerStrip1.FADatePicker.Text + "' and dbo.Table_010_SaleFactor.column02<='" + faDatePickerStrip2.FADatePicker.Text + "')  ", ConSale);
      ...
      , a.Column02 [column03]
      ,sale.[column04]
      ,a2.Column02 [column05]
      ,sale.[column06]
      ,sale.[column07]
      ,sale.[column08]
      ,rd.Column01 [column09]
      ,sh.Column00 [column10]
      ,sale.[column11]
      ,sale.[column12]
      ,sale.[column13]
      ,sale.[column14]
      ...
      ,sale.[Column34]
      ,sale.[Column35]
      ,st.Column02 [Column36]
      ,sale.[Column37]
      ,sale.[Column38]
      ,sale.[Column39]
      ,sale.[Column40]
      ,sale.[Column41]
      ,wh.Column02 [Column42]
      ,sale.[Column43]
      ,sale.[Column44]
      ,sale.[Column45]
      ,sale.[Column46]
      ...
      ,sale.[Column64]
      ,sale.[Column65]
      ,sale.[Column66] ,sale.Column67 from Table_010_SaleFactor sale
        left join " + ConBase.Database + @".dbo.Table_045_PersonInfo a on a.ColumnId=sale.column03
        left join " + ConBase.Database + @".dbo.Table_045_PersonInfo a2 on a2.ColumnId=sale.Column05
        left join " + ConBase.Database + @".dbo.Table_002_SalesTypes st on st.ColumnId=sale.Column36
        left join " + ConWare.Database + @".dbo.Table_007_PwhrsDraft rd on rd.ColumnId=sale.column09
        left join " + ConAcnt.Database + @".dbo.Table_060_SanadHead sh on sh.ColumnId=sale.column10
        left join  Table_018_MarjooiSale ms on ms.ColumnId=sale.Column20
        left join " + ConWare.Database + @".dbo.Table_001_PWHRS wh on wh.ColumnId=sale.Column42
        where sale.columnid=1    ", ConSale);
                HeaderAdapter.Fill(dataSet1, "Header");

                Child1Adapter = new SqlDataAdapter(@"SELECT     dbo.Table_011_Child1_SaleFactor.*    FROM    dbo.Table_011_Child1_SaleFactor where dbo.Table_011_Child1_SaleFactor.column01=1  ", ConSale);
                Child1Adapter.Fill(dataSet1, "Child1");

                Child2Adapter = new SqlDataAdapter(@"SELECT     dbo.Table_012_Child2_SaleFactor.*    FROM   dbo.Table_012_Child2_SaleFactor   where dbo.Table_012_Child2_SaleFactor.column01=1", ConSale);
                Child2Adapter.Fill(dataSet1, "Child2");
...
left join " + ConBase.Database + @".dbo.Table_045_PersonInfo a on a.ColumnId=sale.column03
        ...
        left join  Table_018_MarjooiSale ms on ms.ColumnId=sale.Column20
        left join " + ConWare.Database + @".dbo.Table_001_PWHRS wh on wh.ColumnId=sale.Column42
where sale.Column02>='" + faDatePickerStrip1.FADatePicker.Text + "' and sale.Column02<='" + faDatePickerStrip2.FADatePicker.Text + "' and (sale.Column44=" + projectId + " or '" + (Isadmin) + @"'=N'True') ", ConSale);
                HeaderAdapter.Fill(dataSet1, "Header");

                Child1Adapter = new SqlDataAdapter(@"SELECT     dbo.Table_011_Child1_SaleFactor.*    FROM         dbo.Table_010_SaleFactor INNER JOIN
                dbo.Table_011_Child1_SaleFactor ON dbo.Table_010_SaleFactor.columnid = dbo.Table_011_Child1_SaleFactor.column01 WHERE     (dbo.Table_010_SaleFactor.column02 >='" + faDatePickerStrip1.FADatePicker.Text + "' and dbo.Table_010_SaleFactor.column02<='" + faDatePickerStrip2.FADatePicker.Text + "'  and (dbo.Table_010_SaleFactor.Column44=" + projectId + " or '" + (Isadmin) + @"'=N'True'))", ConSale);
                Child1Adapter.Fill(dataSet1, "Child1");

                Child2Adapter = new SqlDataAdapter(@"SELECT     dbo.Table_012_Child2_SaleFactor.*    FROM         dbo.Table_010_SaleFactor INNER JOIN
                dbo.Table_012_Child2_SaleFactor ON dbo.Table_010_SaleFactor.columnid = dbo.Table_012_Child2_SaleFactor.column01 WHERE     (dbo.Table_010_SaleFactor.column02 >='" + faDatePickerStrip1.FADatePicker.Text + "' and dbo.Table_010_SaleFactor.column02<='" + faDatePickerStrip2.FADatePicker.Text + "' and (dbo.Table_010_SaleFactor.Column44=" + projectId + " or '" + (Isadmin) + @"'=N'True') )", ConSale);
                Child2Adapter.Fill(dataSet1, "Child2");
                ...
                if (this.gridEX_Header.RowCount > 0)
                {
                    if (UserScope.CheckScope(Class_BasicOperation._UserName, "Column11", 20))
                    {
                        foreach (Form item in Application.OpenForms)
                        {
                            ...
                            {
                                item.BringToFront();
                                Frm_002_StoreFaktor frm = (Frm_002_StoreFaktor)item;
                                frm.txt_Search.Text = gridEX_Header.GetRow().Cells["Column01"].Text;
                                frm.bt_Search_Click(sender, e);
                                return;
                            }
                        }
                        Frm_002_StoreFaktor frms = new Frm_002_StoreFaktor(UserScope.CheckScope(Class_BasicOperation._UserName, "Column11", 21), Convert.ToInt32(gridEX_Header.GetValue("columnid")));
                        try
                        {
                    ...
                    if (UserScope.CheckScope(Class_BasicOperation._UserName, "Column11", 128))
                    {
                        if (gridEX_Header.GetCheckedRows().Length == 0)
                        {
                            ...
                            _05_Sale.Reports.Form_SaleFactorPrint1 frm =
                               new Reports.Form_SaleFactorPrint1(int.Parse(gridEX_Header.GetValue("Column01").ToString()), 19);
                            frm.ShowDialog();
                        }
                        else
                        {
                            ...
                            _05_Sale.Reports.Form_SaleFactorPrint1 frm = new Reports.Form_SaleFactorPrint1(List, false);
                            frm.ShowDialog();
                    ...
                    if (this.gridEX_Header.GetCheckedRows().Length > 0)
                    {
                        if (UserScope.CheckScope(Class_BasicOperation._UserName, "Column11", 128))
                        {
                            ...
                            if (CustomerTable.Rows.Count > 0)
                            {
                                ...
                                Double NetTotal = 0, Ezafat = 0, Kosoorat = 0;
        ...
        private void gridEX_Header_FormattingRow(object sender, Janus.Windows.GridEX.RowLoadEventArgs e)
        {
