...
{
    public partial class Form03_FactorAppendix : Form
    {
        int _HeaderId,_HeaderNum;
        
        public Form03_FactorAppendix(int HeaderId, int HeaderNum)
        {
            InitializeComponent();
            _HeaderId = HeaderId;
        ...
        {
            // TODO: This line of code loads data into the 'dataSet_Sale4.Table_66_SaleFactorAttachments' table. You can move, or remove it, as needed.
            this.table_66_SaleFactorAttachmentsTableAdapter.Fill(this.dataSet_Sale4.Table_66_SaleFactorAttachments, _HeaderId);
        ...
        {
            if (this.table_66_SaleFactorAttachmentsBindingSource.Count > 0)
            {
                try
                {
                    this.Validate();
                    this.table_66_SaleFactorAttachmentsBindingSource.EndEdit();
                    this.table_66_SaleFactorAttachmentsTableAdapter.Update(this.dataSet_Sale4.Table_66_SaleFactorAttachments);
...

                {
                    if (DialogResult.Yes == MessageBox.Show("آیا مایل به حذف پیوست(ها) هستید؟", "", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1, MessageBoxOptions.RightAlign))
                    {
                        try
                        {
                            ...
                            {
                                this.table_66_SaleFactorAttachmentsBindingSource.RemoveCurrent();
                                this.table_66_SaleFactorAttachmentsBindingSource.EndEdit();
                                this.table_66_SaleFactorAttachmentsTableAdapter.Update(this.dataSet_Sale4.Table_66_SaleFactorAttachments);
