...
{
    public partial class Form03_PishFactorAppendix : Form
    {
        int _HeaderId,_HeaderNum;

        public Form03_PishFactorAppendix(int HeaderId, int HeaderNum)
        {
            InitializeComponent();
            _HeaderId = HeaderId;
        ...
        {
            this.table_67_PreFactorAttachmentsTableAdapter.Fill(this.dataSet_Sale4.Table_67_PreFactorAttachments, _HeaderId);
        ...
        {
            if (this.table_67_PreFactorAttachmentsBindingSource.Count > 0)
            {
                try
                {
                    this.Validate();
                    this.table_67_PreFactorAttachmentsBindingSource.EndEdit();
                    this.table_67_PreFactorAttachmentsTableAdapter.Update(this.dataSet_Sale4.Table_67_PreFactorAttachments);
...

                {
                    if (DialogResult.Yes == MessageBox.Show("آیا مایل به حذف پیوست(ها) هستید؟", "", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1, MessageBoxOptions.RightAlign))
                    {
                        try
                        {
                            ...
                            {
                                this.table_67_PreFactorAttachmentsBindingSource.RemoveCurrent();
                                this.table_67_PreFactorAttachmentsBindingSource.EndEdit();
                                this.table_67_PreFactorAttachmentsTableAdapter.Update(this.dataSet_Sale4.Table_67_PreFactorAttachments);
