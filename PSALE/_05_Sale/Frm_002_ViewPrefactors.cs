...
{
    public partial class Frm_002_ViewPrefactors : Form
    {
        ...
        private void Frm_002_ViewPrefactors_Load(object sender, EventArgs e)
        {
            ...
            //*******************************************
            HeaderAdapter = new SqlDataAdapter("Select * from Table_007_FactorBefore where Column02>='"+faDatePickerStrip1.FADatePicker.Text+"' and Column02<='"+faDatePickerStrip2.FADatePicker.Text+"'", ConSale);
            HeaderAdapter.Fill(dataSet1,"Header");

            Child1Adapter = new SqlDataAdapter(@"SELECT     dbo.Table_008_Child1_FactorBefore.*    FROM         dbo.Table_007_FactorBefore INNER JOIN
            dbo.Table_008_Child1_FactorBefore ON dbo.Table_007_FactorBefore.columnid = dbo.Table_008_Child1_FactorBefore.column01 WHERE     (dbo.Table_007_FactorBefore.column02 >='" + faDatePickerStrip1.FADatePicker.Text + "' and dbo.Table_007_FactorBefore.column02<='" + faDatePickerStrip2.FADatePicker.Text + "')", ConSale);
            Child1Adapter.Fill(dataSet1, "Child1");

            Child2Adapter = new SqlDataAdapter(@"SELECT     dbo.Table_009_Child2_FactorBefore.*    FROM         dbo.Table_007_FactorBefore INNER JOIN
            dbo.Table_009_Child2_FactorBefore ON dbo.Table_007_FactorBefore.columnid = dbo.Table_009_Child2_FactorBefore.column01 WHERE     (dbo.Table_007_FactorBefore.column02 >='" + faDatePickerStrip1.FADatePicker.Text + "' and dbo.Table_007_FactorBefore.column02<='" + faDatePickerStrip2.FADatePicker.Text + "')", ConSale);
                ...
                if (this.gridEX_Header.RowCount > 0)
                {
                    if (UserScope.CheckScope(Class_BasicOperation._UserName, "Column11", 18))
                    {
                        foreach (Form item in Application.OpenForms)
                        {
                            ...
                            {
                                item.BringToFront();
                                Frm_001_PishFaktor frm = (Frm_001_PishFaktor)item;
                                frm.txt_Search.Text = gridEX_Header.GetRow().Cells["Column01"].Text;
                                frm.bt_Search_Click(sender, e);
                                return;
                            }
                        }
                        Frm_001_PishFaktor frms = new Frm_001_PishFaktor(UserScope.CheckScope(Class_BasicOperation._UserName, "Column11", 19), Convert.ToInt32(gridEX_Header.GetValue("columnid")));
                        try
                        {
        ...
        private void bt_Print_Click(object sender, EventArgs e)
        {
            try
            {
                if (this.gridEX_Header.RowCount > 0)
                {
                    _05_Sale.Reports.Form_PreFactorPrint frm = new Reports.Form_PreFactorPrint
        ...
        private void gridEX_Header_CurrentCellChanged(object sender, EventArgs e)
        {
            try
            {
                txt_Factor.Value = gridEX_List.GetTotalRow().Cells["Column21"].Value.ToString();
