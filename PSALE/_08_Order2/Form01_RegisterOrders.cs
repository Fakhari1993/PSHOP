    ...
    public partial class Form01_RegisterOrders : Form
    {
        ...
        private void Form06_RegisterOrders2_Load(object sender, EventArgs e)
        {
            ...
            try
            {
               ...
               (ConBase.ConnectionString, @"SELECT     dbo.Table_045_PersonInfo.ColumnId AS id, dbo.Table_045_PersonInfo.Column01 AS code, dbo.Table_045_PersonInfo.Column02 AS name, 
                ...
                gridEX_Order.DropDowns[2].DataSource = DS.Tables["Project"];
        ...
        private void gridEX_Order_CellValueChanged(object sender, Janus.Windows.GridEX.ColumnActionEventArgs e)
        {
            ...
            try
            {
                ...
                else if (e.Column.Key == "Column36")
                    Class_BasicOperation.FilterGridExDropDown(sender, "Column36", "Column01", "Column02", gridEX_Order.EditTextBox.Text, Class_BasicOperation.FilterColumnType.ACCColumn);
        ...
        private void gridEX_Order_CellUpdated(object sender, Janus.Windows.GridEX.ColumnActionEventArgs e)
        {
            try
            {
                ...
                Class_BasicOperation.GridExDropDownRemoveFilter(sender, "Column36");
            }
            catch { }
            ...
            try
            {
                ...
                //نام محصول
                if (e.Column.Key == "column02" || e.Column.Key == "column002" || e.Column.Key == "column03" || e.Column.Key == "column04" || e.Column.Key == "column05")
                {
                    ...
                    TotalGoodReservations = Convert.ToDouble(clDoc.TotalGoodReservations(GoodId.ToString(), Convert.ToInt32(Row1["columnid"]), Convert.ToInt16(mlt_WHR.Value)).Rows[0]["Reservations"]);
                ...
                Total = Convert.ToDouble(
                     (Convert.ToDecimal(gridEX_Order.GetValue("column04")) * Convert.ToDecimal(gridEX_Order.GetValue("column28"))) +
                     (Convert.ToDecimal(gridEX_Order.GetValue("column03")) * Convert.ToDecimal(gridEX_Order.GetValue("column29"))) +
                     Convert.ToDecimal(gridEX_Order.GetValue("column05")));
        ...
        private void bt_Save_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (gridEX_Order.GetRows().Count() == 0)
                {
                    Class_BasicOperation.ShowMsg("", "کالایی انتخاب نشده است", "Information");
                    return;
                }

                DataRowView Row = (DataRowView)this.table_005_OrderHeaderBindingSource1.CurrencyManager.Current;

                if (Row["column09"].ToString() == "True" ||
                  Row["column13"].ToString() == "True")
                {
                ...
                foreach (Janus.Windows.GridEX.GridEXRow item in gridEX_Order.GetRows())
                {
                    ...
                    TotalGoodReservations = Convert.ToDouble(clDoc.TotalGoodReservations(item.Cells["Column02"].Value.ToString(), Convert.ToInt32(Row["columnid"]), Convert.ToInt16(mlt_WHR.Value)).Rows[0]["Reservations"]);
        ...
        private void bt_Del_Click(object sender, EventArgs e)
        {
            ...
            {
                DataRowView Row = (DataRowView)this.table_005_OrderHeaderBindingSource1.CurrencyManager.Current;
                if (Row["column09"].ToString() == "True" || Row["Column18"].ToString() == "True")
        ...
        private void mlt_Customer_ValueChanged(object sender, EventArgs e)
        {
            if (((System.Data.DataRowView)(mlt_Customer.SelectedItem)) != null)
            {
                ...
                txt_Address.Text = ((System.Data.DataRowView)(mlt_Customer.SelectedItem)).Row.ItemArray[5].ToString();
            }
...

            {
                try
                {
                    if (table_005_OrderHeaderBindingSource1.Count > 0)
                    {
                        ...
                        {
                            dtOrder.Rows.Add(txt_Num.Text, txt_Date.Text, mlt_Customer.Text, txt_tell.Text, txt_Address.Text, dr.Cells["Column002"].Text, dr.Cells["Column02"].Text, dr.Cells["Column04"].Value, dr.Cells["Column05"].Value, dr.Cells["Column08"].Value, gridEX_Order.GetTotal(gridEX_Order.RootTable.Columns["Column13"], Janus.Windows.GridEX.AggregateFunction.Sum).ToString());
                        }
