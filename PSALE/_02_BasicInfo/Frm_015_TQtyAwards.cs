...
{
    public partial class Frm_015_TQtyAwards : Form
    {
        bool _del;
        bool _BackSpace = false;

        string _FromDate, _ToDate;
        decimal _FromQty = 0, _ToQty = 0;
        int _Kala = -1;
        double _QtyAward = 0;

        public Frm_015_TQtyAwards(bool del)
        {
            _del = del;
            InitializeComponent();
        }
        ...
        private void Frm_005_Javayez_Load(object sender, EventArgs e)
        {
            ...
            this.table_045_TotalQtyAwardsTableAdapter.Fill(
                this.dataSet_05_Awards.Table_045_TotalQtyAwards);
        ...
        private void bt_Save_Click(object sender, EventArgs e)
        {
            ...
            {
                this.Validate();
                this.table_045_TotalQtyAwardsBindingSource.EndEdit();
                this.table_045_TotalQtyAwardsTableAdapter.Update(dataSet_05_Awards.Table_045_TotalQtyAwards);
        ...
        private void bt_Del_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    this.table_045_TotalQtyAwardsBindingSource.RemoveCurrent();
        ...
        private void gridEX1_CellUpdated(object sender, Janus.Windows.GridEX.ColumnActionEventArgs e)
        {
            if (gridEX1.Row == -1)
            {
                ...
                if (gridEX1.Col != gridEX1.RootTable.Columns["column04"].Index)
                    gridEX1.SetValue("column04", _FromQty);
                else
                    _FromQty = Convert.ToDecimal(gridEX1.GetValue("column04"));

                if (gridEX1.Col != gridEX1.RootTable.Columns["column05"].Index)
                    gridEX1.SetValue("column05", _ToQty);
                else
                    _ToQty = Convert.ToDecimal(gridEX1.GetValue("column05"));
